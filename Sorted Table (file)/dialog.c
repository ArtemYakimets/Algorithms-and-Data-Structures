...
const char *errs[] = {"Ok!", "Error: Overflow!", "Error: Key not found!", "Error: Duplicate key!", "Error: File not found!", "Error: File already exists!"};
...
int load_table(Table *table)
{
	...
	if (load(table, name) == FILE_NOT_FOUND)
	{
		printf("Enter the possible vector size:\n");
		if (get_int(&msize) == 0)
			return 0;
		create(table, name, msize);
	}
	free(name);
	return 1;
